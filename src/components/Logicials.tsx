...
export default function Logistics() {
  ...
  const saveNewCount = async (id: string) => {
  const countVal = parseInt(tempCount);
  if (isNaN(countVal)) return toast.error("Please enter a valid number");

  const { data, error } = await supabase
    .from('technical_resources')
    .update({ count: countVal })
    .eq('id', id)
    .select(); 
  ...
  if (!data || data.length === 0) {
    toast.error("Update failed: Check your RLS policies in Supabase");
    console.error("RLS Blocked the update for ID:", id);
  } else {
    toast.success(`Updated ${data[0].name} successfully!`);
    setEditingResourceId(null);
  ...
  const { data, error } = await supabase
    ...
    .select(); // Critical for confirmation

  if (error) {
    // If it says 'exists', it's usually a Duplicate Key error
    toast.error(error.code === '23505' ? "Venue name already exists" : error.message);
  } else if (data) {
    toast.success("Venue added to database");
    setNewVenueName("");
    fetchVenues();
  }
};
