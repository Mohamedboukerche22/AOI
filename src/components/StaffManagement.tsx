...
export default function StaffManagement({ coaches, onAddCoach, newEmail, setNewEmail }: any) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {coaches.map((coach: any) => (
          <div key={coach.id} className="bg-[#111827] border border-gray-800 p-5 rounded-3xl flex items-center gap-4">
            <div className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center text-emerald-500">
              <ShieldCheck size={20} />
            </div>
            <div>
              <p className="font-bold text-white">{coach.full_name || "Coach"}</p>
